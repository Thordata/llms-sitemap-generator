...
def _generate_llms_from_urls(
    ...
    *,
    ...
    if config.output.llms_full_txt:
        write_llms_full(config, rendered_pages, Path(config.output.llms_full_txt))
    if config.output.llms_json:
        write_llms_json(config, rendered_pages, Path(config.output.llms_json))
    if config.output.sitemap_xml:
        ...
        write_sitemap_xml(config, sitemap_urls, config.output.sitemap_xml)
    if config.output.sitemap_index:
        ...
        _write_sitemaps_and_index(config, sitemap_index_urls, Path(config.output.sitemap_index))
